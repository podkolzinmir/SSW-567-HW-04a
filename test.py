...
class TestHw04a(unittest.TestCase):
    def testValidInput2(self):
        with mock.patch('solution.getGitHubInfo', create='gitHubUserId must be a string' ) as MockGithubInfo:
            MockGithubInfo.return_value = 'dangural has the folllowing repos and commits: CPE-695-Final-Project - 0, dsd - 0, Design6 - 1, DSD20S - 1'
            self.assertEqual(MockGithubInfo('dangural'), 'dangural has the folllowing repos and commits: CPE-695-Final-Project - 0, dsd - 0, Design6 - 1, DSD20S - 1')
       
    def testInvalidInput1(self):
        with mock.patch('solution.getGitHubInfo', create='Failing') as MockgetGitHubInfo:
